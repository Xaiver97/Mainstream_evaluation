...
class DeepSVDD:
    ...
    def run(self):
        ...
        # idx = np.load('./batch.npy', allow_pickle=True)
        # cost1 = self.sess.run(self.cost1, feed_dict={self.input_mat: self.train_mat[idx, :],
        # self.input_center: self.center_array})
        # print(cost1)
        # breakpoint()
        # cost2 = self.sess.run(self.cost2, feed_dict={self.input_mat: self.train_mat[idx, :],
        # self.input_center: self.center_array})
        # cost = self.sess.run(self.cost, feed_dict={self.input_mat: self.train_mat[idx, :],
        # self.input_center: self.center_array})
        # print(cost1, '---', cost2, '---', cost)
        # breakpoint()
    ...
    def _prepare_model(self):
        ...
        self.reg_loss += tmp_reg
        self.deno = tf.reduce_mean(self.embeddings ** 2, axis=1, keepdims=True) ** 0.5
        self.deno = tf.where(tf.equal(self.deno, 0), tf.ones_like(self.deno), self.deno)
        # self.embeddings /= (tf.reduce_mean(self.embeddings ** 2, axis=1, keepdims=True) ** 0.5)

        # self.deno = self.deno.numpy()
        # breakpoint()
        # tmp_deno = deno.numpy()
        # tmp_deno[np.where(tmp_deno == 0)] = 1
        # deno = tf.convert_to_tensor(tmp_deno)
        # breakpoint()
        # self.embeddings = tf.reduce_mean(self.embeddings ** 2, axis=1, keepdims=True) ** 0.5
        self.embeddings /= self.deno


        # self.tmp = tf.reduce_sum((self.embeddings - self.input_center) ** 2, axis=1)
        # self.tmp = tf.Print(self.tmp, [self.tmp], 'tmp is ')


        self.cost1 = tf.reduce_mean(tf.reduce_sum((self.embeddings - self.input_center) ** 2, axis=1))
        # self.cost1 = tf.reduce_sum((self.embeddings - self.input_center) ** 2, axis=1)
    ...
    def test_model(self):
        ...
        with open('../mainstream_score/' + self.args.data + '/MS_DeepSVDD.npy', "wb") as f:
            np.save(f, -distance)
...
with open('../../mod_data/data_statistic.pkl', 'rb') as f:
    info = pickle.load(f)
    args.num_user = info[args.data][0]
    args.num_item = info[args.data][1]

train_df = pd.read_csv('../../mod_data/' + args.data + '/sep_data/train_df.csv')

# train_like = list(np.load('../../datasets/' + args.data + '/sep_data/user_train_like.npy', allow_pickle=True))
# test_like = list(np.load('../../datasets/' + args.data + '/sep_data/user_test_like.npy', allow_pickle=True))
# Rec = np.load('../../datasets/' + args.data + '/sep_data/Res_VAE.npy')
...
with tf.Session() as sess:
    autorec = DeepSVDD(sess, args, train_df)
    autorec.run()
